...
**Decisão**: Normalizar tudo para `CanonicalPost` no ponto de ingestão, antes da persistência. `imageUrl` e `videoUrl` entram como URLs canônicas do post, não como bytes de mídia baixados.  
**Consequência**: A UI e persistência nunca lidam com dados crus de protocolo. A ingestão guarda referências de mídia; o carregamento real de imagem/vídeo fica sob demanda na renderização. Ver `docs/ingestion-to-post-flow.md`.

## ADR-004: IndexedDB como persistência padrão

**Contexto**: Precisamos de storage local que funcione em todas as plataformas web.  
**Decisão**: IndexedDB como engine primária. Database `notfeed-v2`, version `18`. SQLite via Tauri plugin como melhoria futura para desktop. Migração destrutiva: ao incrementar a versão, todos os stores são deletados e recriados (sem lógica de migrations incrementais durante pré-lançamento).  
...
**Decisão**: Usar a infra nativa do SvelteKit + Workbox direto (`workbox-precaching`, `workbox-routing`). O SW é escrito manualmente em `src/service-worker.ts` e consome `build`, `files`, `prerendered`, `version` do módulo virtual `$service-worker` para construir o manifesto do `precacheAndRoute()`. Uma `NavigationRoute` com `createHandlerBoundToURL('/')` serve a SPA shell para qualquer navegação in-scope. Em `svelte.config.js`, `kit.serviceWorker.register = false` desliga o auto-registro; o registro é feito manualmente em `src/lib/platform/web/sw-register.ts` e gated por `capabilities.platform !== 'desktop'` no `+layout.svelte`. Os listeners `sync`, `periodicsync` e `message` chamam o `PostManager.tick()` isomórfico. O SW também mantém um runtime cache cache-first para imagens requisitadas pela UI, com expiração por 30 dias sem uso. Manifest é arquivo estático (`static/manifest.webmanifest`) linkado em `app.html`.  
**Consequência**: Sem dependência de `@vite-pwa/sveltekit` (rejeitado em ADR-033 por supply-chain). Total de deps PWA: `workbox-precaching`, `workbox-routing`, `workbox-window`, `sharp`. SW emitido em `build/service-worker.js`. Plano AC desabilita o registro automaticamente via `capabilities.platform === 'desktop'`, sem precisar de env var de build. Imagens de posts continuam carregadas sob demanda; quando vistas, podem ser reabertas sem redownload enquanto permanecerem no cache runtime.
...
## ADR-044: Mídia de posts como URLs na ingestão e bytes sob demanda

**Contexto**: RSS, Atom, JSON Feed e Nostr podem apontar para imagens e vídeos de formas diferentes. Era importante capturar essas mídias durante a ingestão para que o post já nasça completo, mas sem transformar a ingestão em pré-carregamento pesado de arquivos externos.
**Decisão**: Clients capturam hints específicos do protocolo (`media:*`, enclosures, attachments, tags Nostr); normalizers escolhem/canonicalizam `imageUrl` e `videoUrl` com fallback genérico sobre HTML/texto já presente no payload; `savePostsForUser` persiste essas URLs no `CanonicalPost`. A UI carrega os bytes apenas quando renderiza `<img>`, `<video>` ou iframe. Imagens requisitadas passam pelo runtime cache do service worker (`request.destination === 'image'`), com estratégia cache-first, limite de entradas e expiração por 30 dias sem uso. Vídeos permanecem sob demanda sem cache persistente próprio do Notfeed.
**Consequência**: A ingestão continua leve e protocol-agnostic no formato final: posts podem ficar salvos com `imageUrl`/`videoUrl` sem baixar mídia. O usuário paga o custo de rede apenas ao visualizar, e imagens já vistas podem ser reabertas localmente. O fluxo completo está documentado em `docs/ingestion-to-post-flow.md`.
