...
## ADR-038: Idle tiers e backoff totalmente configuráveis per-usuário

**Contexto**: Hardcodar valores como "24h vira tier idle 1" e "depois de N falhas espera M minutos" não atendia usuários com perfis diferentes (workstation always-on vs. mobile esporádico). Pedido explícito do PO: tornar didático e configurável.  
**Decisão**: `UserSettings.ingestion` expõe 5 campos para tiers (`idleTier1IntervalMs`, `idleTier1MaxIdleMs`, `idleTier2IntervalMs`, `idleTier2MaxIdleMs`, `idleTier3IntervalMs`) e 4 campos para backoff (`backoffEnabled`, `backoffMultiplier`, `maxBackoffSteps`, `maxBackoffMs`). A página `/user/settings/ingestion` apresenta os tiers em **layout de tabela** (Tier · Período · Intervalo) com o limite de cada tier aparecendo tanto como input editável na linha N quanto como texto de "início" da linha N+1, reforçando o encadeamento. O backoff tem um Switch que esconde os inputs quando desabilitado e um **preview ao vivo** (`$derived.by`) das primeiras N falhas (ex.: `#1: 10min · #2: 20min · #3: 40min ...`) recalculado conforme o usuário ajusta os campos. Fórmula: `min(intervalo * multiplier^min(failures, maxSteps), maxBackoffMs)`.  
**Consequência**: Zero defaults hardcoded em `pickIntervalForUser` / `pickBackoffConfig`; tudo vem do `UserSettings` do usuário. Defaults compatíveis com o comportamento anterior (tier1 30min/24h, tier2 4h/72h, tier3 24h; backoff 2× até 6 steps, teto 24h). `IDLE_TIER_THRESHOLD_MS` foi removido. UX validada com preview e hints em linguagem simples nas duas línguas (en-US, pt-BR).
