...
- **Desktop**: Tauri v2
- **Persistência**: IndexedDB (`notfeed-v2`, v18) atrás da abstração `StorageBackend`
...
24. ✅ Fluxo de mídia de posts: ingestão/normalização persistem `imageUrl` e `videoUrl` sem baixar os arquivos; UI carrega mídia sob demanda; imagens vistas usam cache runtime com expiração por 30 dias sem uso. Ver `docs/ingestion-to-post-flow.md`.
