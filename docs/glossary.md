...
| **Back-settings** | Arquivo `src/lib/config/back-settings.ts` com knobs de sistema que o desenvolvedor ajusta a nível de código (não expostos na UI). Atualmente contém `INGESTION_BACKOFF`; reservado para futuros rate limits, timeouts default e feature flags de experimentos. |
