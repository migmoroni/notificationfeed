...
<svelte:head>
	{#if preconnected}
		...
		<link rel="preconnect" href="https://dailymotion.com" />
	{/if}
</svelte:head>
...
{:else}
	<div class="absolute inset-0 z-0 bg-gradient-to-br from-sky-900 via-sky-800 to-slate-900"></div>
{/if}

{#if !iframeLoaded}
	<button
		class="absolute inset-0 z-10 w-full h-full border-none p-0 bg-black/15 flex items-center justify-center cursor-pointer group/facade focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 hover:bg-black/5 transition-colors"
		type="button"
	...
	>
		<div class="relative flex items-center gap-3 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-white transition-transform duration-200 group-hover/facade:scale-105 group-focus-visible/facade:scale-105">
			...
			<span class="text-sm font-medium tracking-wide">Dailymotion</span>
		</div>
	</button>
{:else}
	<iframe
